...
private:
    ...
    void createVertexBuffers()
    {
        ...
        vmaCreateBuffer(vmaAllocator, &bufferInfo, &allocInfo, &vertexBuffer, &vertexBufferAllocation, nullptr);
        copyBufferToGPU(stagingBuffer, vertexBuffer, bufferSize);
    ...
    void createIndexBuffer()
    {
        ...
        bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        allocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
        vmaCreateBuffer(vmaAllocator, &bufferInfo, &allocInfo, &indexBuffer, &indexBufferAllocation, nullptr);
        copyBufferToGPU(stagingBuffer, indexBuffer, bufferSize);
        vmaDestroyBuffer(vmaAllocator, stagingBuffer, allocation);
    }
    ...
    void createCommandPool()
    {
        ...
        poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamily.value();
        if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPoolTransient) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create command pool!");
        }
    }
