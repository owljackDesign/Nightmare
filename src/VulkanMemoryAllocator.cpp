...
private:
    ...
    VkDeviceMemory genericMemoryCPU;
    std::map<VkBuffer, uint32_t> bufferMemoryMapCPU;
    VkDeviceMemory genericMemoryGPU;
    std::map<VkBuffer, uint32_t> bufferMemoryMapGPU;


    void createBufferMemoryPools()
    {
        VkMemoryAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        allocInfo.allocationSize = 1024 *1000;
        allocInfo.memoryTypeIndex = findMemoryType(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        if (vkAllocateMemory(device, &allocInfo, nullptr, &genericMemoryCPU) != VK_SUCCESS) 
        {
            throw std::runtime_error("failed to allocate buffer memory!");
        }
        allocInfo.memoryTypeIndex = findMemoryType(VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        if (vkAllocateMemory(device, &allocInfo, nullptr, &genericMemoryGPU) != VK_SUCCESS) 
        {
            throw std::runtime_error("failed to allocate buffer memory!");
        }
    }

    void createBufferCPU(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer)
    {
        ...
        uint32_t currMemMappingVal = 0;
        for (auto buff = bufferMemoryMapCPU.begin(); buff != bufferMemoryMapCPU.end(); buff++)
        {
            currMemMappingVal += buff->second;
        }
        vkBindBufferMemory(device, buffer, genericMemoryCPU, currMemMappingVal);
        bufferMemoryMapCPU.emplace(buffer, static_cast<uint32_t>(size));
    }

    void createBufferGPU(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer)
    {
        VkBufferCreateInfo bufferInfo{};
        bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        bufferInfo.size = size;
        ...
        uint32_t currMemMappingVal = 0;
        for (auto buff = bufferMemoryMapGPU.begin(); buff != bufferMemoryMapGPU.end(); buff++)
        {
            currMemMappingVal += buff->second;
        }
        vkBindBufferMemory(device, buffer, genericMemoryGPU, currMemMappingVal);
        bufferMemoryMapGPU.emplace(buffer, static_cast<uint32_t>(size));
    }
    ...
    bool isDeviceSuitable(const VkPhysicalDevice &device)
    {
        ...
        return indicies.isComplete() && extensionsSupported && swapChainAdq;
    }
